...
class TestCppColorExtractor:
    ...
    def test_cpp_color_extractor_module_exists(self):
        """测试 C++ 颜色提取器模块存在"""
        module_path = Path('freeassetfilter/core/native/src/cpp_color_extractor/__init__.py')
        assert module_path.exists(), "C++ 颜色提取器模块不存在"
    
    def test_cpp_color_extractor_setup_exists(self):
        """测试 C++ 颜色提取器 setup 文件存在"""
        setup_path = Path('freeassetfilter/core/native/src/cpp_color_extractor/setup.py')
        assert setup_path.exists(), "C++ 颜色提取器 setup.py 不存在"
    
    def test_cpp_color_extractor_can_import(self):
        """测试 C++ 颜色提取器可以导入"""
        try:
            from freeassetfilter.core.native.src import cpp_color_extractor
            assert True
    ...
    def test_color_extractor_class(self):
        """测试 ColorExtractor 类"""
        try:
            from freeassetfilter.core.native.src.cpp_color_extractor import ColorExtractor
...
class TestCppLutPreview:
    ...
    def test_cpp_lut_preview_module_exists(self):
        """测试 C++ LUT 预览模块存在"""
        module_path = Path('freeassetfilter/core/native/src/cpp_lut_preview/__init__.py')
        assert module_path.exists(), "C++ LUT 预览模块不存在"
    
    def test_cpp_lut_preview_setup_exists(self):
        """测试 C++ LUT 预览 setup 文件存在"""
        setup_path = Path('freeassetfilter/core/native/src/cpp_lut_preview/setup.py')
        assert setup_path.exists(), "C++ LUT 预览 setup.py 不存在"
    
    def test_cpp_lut_preview_can_import(self):
        """测试 C++ LUT 预览可以导入"""
        try:
            from freeassetfilter.core.native.src import cpp_lut_preview
            assert True
        except ImportError as e:
...
class TestCppExtensionIntegration:
    ...
    def test_cpp_extensions_directory_structure(self):
        """测试 C++ 扩展目录结构"""
        cpp_dirs = [
            'freeassetfilter/core/native/src/cpp_color_extractor',
    ...
    def test_cpp_extensions_have_source_files(self):
        """测试 C++ 扩展有源文件"""
        # 检查颜色提取器源文件
        color_extractor_sources = list(Path('freeassetfilter/core/native/src/cpp_color_extractor').glob('*.cpp'))
        color_extractor_sources += list(Path('freeassetfilter/core/native/src/cpp_color_extractor').glob('*.c'))
        
        # 检查 LUT 预览源文件
        lut_preview_sources = list(Path('freeassetfilter/core/native/src/cpp_lut_preview').glob('*.cpp'))
        lut_preview_sources += list(Path('freeassetfilter/core/native/src/cpp_lut_preview').glob('*.c'))
        ...
        # 或者检查是否有已编译的 pyd 文件
        has_compiled = (
            list(Path('freeassetfilter/core/native/src/cpp_color_extractor').glob('*.pyd')) or
            list(Path('freeassetfilter/core/native/src/cpp_lut_preview').glob('*.pyd'))
        )
