...
def test_comprehensive():
    ...
    debug("调用file_selector.dropEvent() - 拖拽添加")
    file_selector.dropEvent(drop_event)
    
    # 等待异步操作
    import time
    for i in range(5):  # 等待0.5秒
        app.processEvents()
        time.sleep(0.1)
    
    debug(f"拖拽后 - 储存池项目数: {len(staging_pool.items)}")
    debug(f"拖拽后 - 收到信号数: {len(signals_received)}")
    
    # 再次拖拽同一个文件（测试重复添加）
    debug(f"再次拖拽同一个文件: {file_path} - 测试重复添加")
    ...
    debug("调用file_selector.dropEvent() - 第二次拖拽")
    ...
    debug(f"第二次拖拽后 - 储存池项目数: {len(staging_pool.items)}")
    debug(f"第二次拖拽后 - 收到信号数: {len(signals_received)}")
    
    # 设置文件选择器到包含临时文件的目录以便点击测试
    ...
    if temp_card is not None:
        debug(f"找到临时文件卡片，开始点击测试")
        
        # 多次切换选中状态
        for i in range(3):
            debug(f"第{i+1}次切换选中状态")
            temp_card.set_selected(not temp_card.is_selected())
            
            # 等待异步操作
            for j in range(2):  # 等待0.2秒
                app.processEvents()
                time.sleep(0.1)
            
            debug(f"第{i+1}次切换后 - 储存池项目数: {len(staging_pool.items)}")
    ...
    debug(f"综合测试结果: {'成功' if success else '失败'}")
...
if __name__ == "__main__":
    ...
    debug(f"综合测试完成: {'成功' if success else '失败'}")
