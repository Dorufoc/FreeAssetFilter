...
def debug(msg):
    ...
    print(f"[{timestamp}] [右键点击测试] {msg}")

def test_right_click_selection():
    """测试右键点击切换选中状态"""
    debug("=== 开始右键点击测试 ===")
    ...
    # 第一次右键点击（选中文件）
    debug("第一次右键点击（选中文件）")
    temp_card.set_selected(True)  # 直接调用set_selected来触发selection_changed信号
    ...
    debug(f"第一次右键点击后 - 储存池项目数: {len(staging_pool.items)}")
    debug(f"第一次右键点击后 - 收到信号数: {len(signals_received)}")
    
    # 第二次右键点击（取消选中文件）
    debug("第二次右键点击（取消选中文件）")
    temp_card.set_selected(False)  # 取消选中
    ...
    debug(f"第二次右键点击后 - 储存池项目数: {len(staging_pool.items)}")
    debug(f"第二次右键点击后 - 收到信号数: {len(signals_received)}")
    
    # 第三次右键点击（重新选中文件）
    debug("第三次右键点击（重新选中文件）")
    temp_card.set_selected(True)  # 重新选中
    ...
    debug(f"第三次右键点击后 - 储存池项目数: {len(staging_pool.items)}")
    debug(f"第三次右键点击后 - 收到信号数: {len(signals_received)}")
    
    # 检查最终状态
    expected_items = 1  # 应该只有一个项目（即使多次切换，最终只保留一个选中状态）
    actual_items = len(staging_pool.items)
    duplicate_detected = actual_items > 1
    ...
    # 检查是否合理（虽然可能有多个信号，但最终项目数应该正确）
    success = actual_items == 1  # 最终应该只有1个项目
...
if __name__ == "__main__":
    debug("开始右键点击测试")
    success = test_right_click_selection()
