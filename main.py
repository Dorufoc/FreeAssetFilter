...
def install_missing_dependencies(missing_deps):
    ...
    cmd = [sys.executable, "-m", "pip", "install", "-i", "https://pypi.tuna.tsinghua.edu.cn/simple/", "--upgrade"] + missing_deps
...
def install_vlc_windows():
    ...
    vlc_installer_path = os.path.join(program_dir, "vlc-3.0.21-win64.exe")
    vlc_install_path = os.path.join(program_dir, "vlc")
    
    # 检查VLC是否已安装在程序目录
    if os.path.exists(vlc_install_path) and os.path.exists(os.path.join(vlc_install_path, "vlc.exe")):
    ...
    try:
        import requests
        url = "https://mirrors.tuna.tsinghua.edu.cn/videolan-ftp/vlc/3.0.21/win64/vlc-3.0.21-win64.exe"
    ...
    try:
        ...
        # 验证安装
        if os.path.exists(os.path.join(vlc_install_path, "vlc.exe")):
            print(f"VLC已成功安装到: {vlc_install_path}")
            return True
        else:
            print(f"VLC安装失败，未找到vlc.exe")
            return False
    except subprocess.CalledProcessError as e:
        print(f"VLC安装失败:\n{e.stderr}")
        return False
...
if __name__ == "__main__":
    ...
    if not check_vlc_installed():
        ...
        if sys.platform == "win32":
            ...
            print("     - CentOS/RHEL: sudo yum install vlc")
