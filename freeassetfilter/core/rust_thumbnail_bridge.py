...
class RustThumbnailBridge:
    ...
    def _load(self):
        ...
        for path in self._candidate_paths():
            ...
            try:
                ...
                debug(f"[RustThumbnailBridge] 已加载原生引擎: {path}")
                return
            except Exception as e:
                warning(f"[RustThumbnailBridge] 加载失败 {path}: {e}")
        self._available = False
        self._dll = None
        warning("[RustThumbnailBridge] 未找到可用的 thumbnail_generator.dll，将回退 Python 实现")
    ...
    def set_cache_limit(self, max_bytes: int) -> bool:
        ...
        try:
            ...
            warning(f"[RustThumbnailBridge] set_cache_limit 失败: {e}")
            return False
    ...
    def generate_rgba(self, file_path: str, width: int, height: int) -> Optional[Tuple[bytes, int, int, int]]:
        ...
        try:
            ...
            warning(f"[RustThumbnailBridge] generate_rgba 失败: {e}")
            return None
    ...
    def generate_jpeg(self, file_path: str, width: int, height: int) -> Optional[bytes]:
        ...
        try:
            ...
            warning(f"[RustThumbnailBridge] generate_jpeg 失败: {e}")
            return None
    ...
    def generate_jpg(self, file_path: str, width: int, height: int) -> Optional[bytes]:
        ...
        try:
            ...
            warning(f"[RustThumbnailBridge] generate_jpg 失败: {e}")
            return None
    ...
    def generate_jpg_batch(self, file_paths: List[str], width: int, height: int) -> List[Optional[bytes]]:
        ...
        try:
            ...
            warning(f"[RustThumbnailBridge] generate_jpg_batch 失败: {e}")
            return outputs
        finally:
