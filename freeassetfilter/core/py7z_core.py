...
class Py7zCore:
    ...
    def _run_7z_command(self, args: List[str], encoding: str = "utf-8") -> Tuple[int, str, str]:
        """
        运行 7z 命令
    ...
    def _parse_file_block(self, block: str) -> Optional[Dict]:
        ...
        # 提取 Modified 时间
        modified_match = re.search(r'^Modified = (.+)$', block, re.MULTILINE)
        if modified_match:
        ...
        attr_match = re.search(r'^Attributes = (.+)$', block, re.MULTILINE)
        if attr_match:
    ...
    def get_archive_type(self, archive_path: str) -> str:
        ...
        # 解析类型信息
        # 7z 输出格式中通常包含 "Type = " 行
        type_match = re.search(r'^Type = (.+)$', stdout, re.MULTILINE)
        if type_match:
