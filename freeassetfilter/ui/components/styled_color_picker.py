...
class _ColorPanel(QFrame):
    ...
    带淡入淡出动画的 Qt.Tool 窗口，使用 WA_TranslucentBackground
    实现四角透明的圆角卡片效果。Qt.Tool + WA_ShowWithoutActivating
    防止工具窗口抢焦点并避免 Windows 矩形原生阴影。
    """
    ...
    def __init__(self, parent=None):
        # 使用 Qt.Tool + FramelessWindowHint + WindowDoesNotAcceptFocus
        # Tool 窗口：无任务栏入口、不抢主窗口焦点
        # FramelessWindowHint：无边框、无标题栏
        # WindowDoesNotAcceptFocus：不接收键盘焦点
        ...
        # WA_ShowWithoutActivating：显示时不激活窗口、不抢焦点
    ...
    def is_closing(self) -> bool:
        """Return True if the panel is in the closing animation phase."""
        return self._closing

    def reopen(self) -> None:
        """Cancel an in-progress close animation and fade back in.

        This is used to reverse a closing animation when the user clicks
        the toggle button during fade-out. The panel remains visible but
        transitions from fading-out to fading-in.

        Precondition: is_closing must be True.
        Postcondition: is_closing becomes False, opacity animates toward 1.0.
        """
        ...
        # Start fade-in from current opacity
    ...
    def hideEvent(self, event):
        ...
        确保动画停止，关闭标志重置，closed 信号发射（仅一次）。
        """
        # 停止动画
        if self._opacity_anim.state() != QPropertyAnimation.Stopped:
            self._opacity_anim.stop()
