...
- 整个控件只做一层不透明绘制：``paintEvent`` 内只允许单次 ``fillRect``
  （ambient 纯色、交互态快速渐变、降级态）或单次 ``drawImage``
  （稳定态抖动缓存图）。
- 禁止多层叠加：不得先后多次 ``fillRect`` / ``drawPixmap`` / 叠加半透明层，
  不得在绘制期用 ``setOpacity(<1)`` 或叠加第二层。
...
from PySide6.QtGui import QBrush, QColor, QImage, QLinearGradient, QPainter, QPaintEvent
...
class MinimalistBackgroundWidget(QWidget):
    ...
    def sync_theme(self) -> None:
        """主题切换同步入口：清抖动缓存并触发一次重绘。"""
        self._dither_cache.clear()
        self.update()
